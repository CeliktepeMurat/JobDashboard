...
// Feed application — an application linked to a Job from the DB feed
...
// Manual application — added directly by the user
  ...
  useEffect(() => {
    async function load() {
      const [appsRes, manualRes] = await Promise.all([
        fetch("/api/applications?include=job"),
  ...
  return (
    <div className="flex flex-col gap-8">
      <div className="flex items-center justify-between">
        <div>
          <h1 className="text-xl font-semibold text-zinc-900">Tracker</h1>
        ...
        <button
          ...
          className="text-sm font-medium px-3 py-1.5 rounded bg-zinc-900 text-white hover:bg-zinc-700 transition-colors"
        >
          + Add Manual Job
        </button>
      </div>
      ...
      <section>
        ...
        {!loading && feedApps.length === 0 && (
          <p className="text-sm text-zinc-400">
            ...
            <a href="/" className="underline hover:text-zinc-600">Feed</a>.
          </p>
        )}
        <div className="flex flex-col gap-2">
          {feedApps.map((app) => (
            <div
              key={app.id}
              className="bg-white border border-zinc-200 rounded-lg px-4 py-3 flex items-start justify-between gap-4"
            >
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center gap-2 flex-wrap text-sm text-zinc-500 mt-0.5">
                ...
                {app.notes && (
                  <p className="text-xs text-zinc-400 mt-1">{app.notes}</p>
                )}
              </div>
            ...
            >
              <div className="flex-1 min-w-0">
                <div className="font-medium text-zinc-900">
                  {app.role}{" "}
                  <span className="font-normal text-zinc-500">@ {app.company}</span>
                </div>
                <div className="flex items-center gap-2 flex-wrap text-sm text-zinc-500 mt-0.5">
